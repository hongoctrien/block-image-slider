...
if (! nv_function_exists('nv_block_image_slider')) {
    ...
    function nv_block_image_slider_config($module, $data_block, $lang_block)
    {
        ...
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.image_slider.tpl')) {
            $block_theme = $global_config['site_theme'];
        } else {
            $block_theme = 'default';
        }
        
        $xtpl = new XTemplate('global.image_slider.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
        ...
        foreach($block_config['image'] as $index => $image){
            ...
            if(nv_is_url($image['link'])){
                if(!empty($image['description'])){
                    $xtpl->parse('main.image.link.caption');
                }
                $xtpl->parse('main.image.link');
            }else{
                if(!empty($image['description'])){
                    $xtpl->parse('main.image.nolink.caption');
                }
